    ...
    while True: # 全てのページを巡回するためのループ
        try:
            ...
            for row in rows:
                ...
                if company_name.startswith('Ｅ－') or company_name.startswith('Ｐ－') or company_name.startswith('Ｒ－'):
                    print(f"  -> スキップ (除外-社名): {company_name}")
                    continue
                if '上場投信' in title:
                    print(f"  -> スキップ (除外-表題): {title}")
                    continue
                if 'ＥＴＦ' in title.upper() or 'ETF' in title.upper():
                    print(f"  -> スキップ (除外-表題): {title}")
                    continue
                if '上場ETN' in title:
                    print(f"  -> スキップ (除外-表題): {title}")
...
def main():
    """メイン処理"""
    print("TDnet Downloader を起動します。")

    date_input = input("ダウンロードしたい日付を YYMMDD 形式で入力してください (例: 250827)。\nEnterキーのみの場合は本日の日付で実行します: ")

    target_date = None
    if not date_input:
        target_date = datetime.now()
        print(f"日付が入力されなかったため、本日 ({target_date.strftime('%Y年%m月%d日')}) のデータを取得します。")
    else:
        try:
            target_date = datetime.strptime(date_input, '%y%m%d')
            print(f"指定された日付 ({target_date.strftime('%Y年%m月%d日')}) のデータを取得します。")
        except ValueError:
            print(f"エラー: '{date_input}' は正しい日付形式 (YYMMDD) ではありません。プログラムを終了します。")
            return
